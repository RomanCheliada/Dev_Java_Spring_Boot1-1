...
public class CargoViewRepositoryImpl implements CargoViewRepository{
	...
	private static class PredicateBuilder {
		...
		void filterAll(){
			predicates.add(root.get(Cargo_.transporter).isNull());
		}
		
		void filterByMinWeight(){
			if(!filter.getMinWeight().isEmpty()){
				predicates.add(cb.ge(root.get(Cargo_.weight), Integer.valueOf(filter.getMinWeight())));
			}
		}
		
		void filterByMaxWeight(){
			if(!filter.getMaxWeight().isEmpty()){
				predicates.add(cb.le(root.get(Cargo_.weight), Integer.valueOf(filter.getMaxWeight())));
			}
		}
		
		void filterByMinHeight(){
			if(!filter.getMinHeight().isEmpty()){
				predicates.add(cb.ge(root.get(Cargo_.height), Integer.valueOf(filter.getMinHeight())));
			}
		}
		
		void filterByMaxHeight(){
			if(!filter.getMaxHeight().isEmpty()){
				predicates.add(cb.le(root.get(Cargo_.height), Integer.valueOf(filter.getMaxHeight())));
			}
		}
		
		void filterByMinWidth(){
			if(!filter.getMinWidth().isEmpty()){
				predicates.add(cb.ge(root.get(Cargo_.width), Integer.valueOf(filter.getMinWidth())));
			}
		}
		
		void filterByMaxWidth(){
			if(!filter.getMaxWidth().isEmpty()){
				predicates.add(cb.le(root.get(Cargo_.width), Integer.valueOf(filter.getMaxWidth())));
			}
		}
		
		void filterByMinLength(){
			if(!filter.getMinLength().isEmpty()){
				predicates.add(cb.ge(root.get(Cargo_.length), Integer.valueOf(filter.getMinLength())));
			}
		}
		
		void filterByMaxLength(){
			if(!filter.getMaxLength().isEmpty()){
				predicates.add(cb.le(root.get(Cargo_.length), Integer.valueOf(filter.getMaxLength())));
			}
		}
		
		void findByCityFrom(){
			if(!filter.getCityFrom().isEmpty()){
				Join<Cargo, City> cityFromJoin = root.join(Cargo_.cityFrom);
				predicates.add(cityFromJoin.get(City_.name).in(filter.getCityFrom()));
			}
		}
		
		void findByCityTo(){
			if(!filter.getCityTo().isEmpty()){
				Join<Cargo, City> cityToJoin = root.join(Cargo_.cityTo);
				predicates.add(cityToJoin.get(City_.name).in(filter.getCityTo()));
			}
		}
