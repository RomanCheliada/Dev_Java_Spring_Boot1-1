	...
	private static class PredicateBuilder {
		...
		void filterByMinCount() {
			if(!filter.getMinCount().isEmpty()) {
				predicates.add(cb.ge(root.get(Owner_.count), Integer.valueOf(filter.getMinCount())));
			}
		}

		void filterByMaxCount() {
			if(!filter.getMaxCount().isEmpty()) {
				predicates.add(cb.le(root.get(Owner_.count), Integer.valueOf(filter.getMaxCount())));
			}
		}
