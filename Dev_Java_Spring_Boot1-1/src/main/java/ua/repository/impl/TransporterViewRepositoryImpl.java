...
import ua.entity.Brand_;
import ua.entity.City;
import ua.entity.City_;
import ua.entity.Model;
import ua.entity.Model_;
	...
	private static class PredicateBuilder {
		...
		void filterByMinRate() {
			if(!filter.getMinRate().isEmpty()) {
				predicates.add(cb.ge(root.get(Transporter_.rate), new BigDecimal(filter.getMinRate().replace(',', '.'))));
			}
		}

		void filterByMaxRate() {
			if(!filter.getMaxRate().isEmpty()) {
				predicates.add(cb.le(root.get(Transporter_.rate), new BigDecimal(filter.getMaxRate().replace(',', '.'))));
			}
		}

		void filterByMinWeight() {
			if(!filter.getMinWeight().isEmpty()) {
				predicates.add(cb.ge(root.get(Transporter_.maxWeight), Integer.valueOf(filter.getMinWeight())));
			}
		}

		void filterByMaxWeight() {
			if(!filter.getMaxWeight().isEmpty()) {
				predicates.add(cb.le(root.get(Transporter_.maxWeight), Integer.valueOf(filter.getMaxWeight())));
			}
		}
		
		void filterByMinCount() {
			if(!filter.getMinCount().isEmpty()) {
				predicates.add(cb.ge(root.get(Transporter_.count), Integer.valueOf(filter.getMinCount())));
			}
		}

		void filterByMaxCount() {
			if(!filter.getMaxCount().isEmpty()) {
				predicates.add(cb.le(root.get(Transporter_.count), Integer.valueOf(filter.getMaxCount())));
			}
		}
		
		void filterByMinAge() {
			if(!filter.getMinAge().isEmpty()) {
				predicates.add(cb.ge(root.get(Transporter_.age), Integer.valueOf(filter.getMinAge())));
			}
		}

		void filterByMaxAge() {
			if(!filter.getMaxAge().isEmpty()) {
				predicates.add(cb.le(root.get(Transporter_.age), Integer.valueOf(filter.getMaxAge())));
			}
		}
		...
		void findByModelId() {
			if(!filter.getModelIds().isEmpty()) {
				...
				predicates.add(modelJoin.get(Model_.name).in(filter.getModelIds()));
			}
		}
		
		void findByBrandId() {
			if(!filter.getBrandIds().isEmpty()) {
				if(modelJoin==null) modelJoin = root.join(Transporter_.model);
				Join<Model, Brand> brandJoin = modelJoin.join(Model_.brand);
				predicates.add(brandJoin.get(Brand_.name).in(filter.getBrandIds()));
			}
		}
